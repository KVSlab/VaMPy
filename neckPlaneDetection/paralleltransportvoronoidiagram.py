...
def VoronoiDiagramInterpolation(interpolationcellid, id0, id1, voronoiDataset0,
    ...
    if distance(endPoint, cellLine.GetPoint(endId)) > 0.5:
        id1 = 1 if id1 == 2 else 2
        endPoint = clippingPoints.GetPoint(id1)
        endId = cellLine.FindPoint(endPoint)
...
def remove_extreme_points(new, old, dist_tol=1.0):
    """Removes extreme spheres in the new voronoi diagram.
    The first criteria is if a point has a radius lager then 1.1 times the
    largest radius in the clipped voronoi diagram.
    The second criteria is that the distance to the closest other Voronoi
    diagram point is larger than dist_tol [mm]"""

    # Mark points which are larger
    tolerance = get_array(radiusArrayName, old).max() * 1.1
    marked = get_array(radiusArrayName, new) <= tolerance

    # Mark points which are to far away
    locator = get_locator(new)

    for i in range(new.GetNumberOfPoints()):
        id_list = vtk.vtkIdList()
        point = new.GetPoint(i)
        locator.FindClosestNPoints(2, point, id_list)
        id1 = id_list.GetId(0)
        id2 = id_list.GetId(1)
        closest = id1 if id2 == i else id1
        dist = distance(point, new.GetPoint(closest))
        if dist > dist_tol:
            marked[i] = 0

    # Remove marked voronoi points
    radiusArray = get_array(radiusArrayName, new)[marked]
    radiusArray = create_vtk_array(radiusArray, radiusArrayName)
...
def interpolate_voronoi_diagram(interpolatedCenterlines, patchCenterlines,
    ...
    print("Number of points in V-d", clippedVoronoi.GetNumberOfPoints())
    ...
    print("Number of points in V-d", completeVoronoiDiagram.GetNumberOfPoints())
