...
def get_teams(dirpath):
    ...
    for folder in files:
        try:
            tmp = folder.replace("a", ".0").replace("b", ".5")
            a = float(tmp)
            teams.append(path.join(dirpath, folder))
        except:
            pass

    teams.sort(key=lambda x: float(x.replace("a", ".0").replace("b", ".5").split(path.sep)[-1]))
...
def weighted_mean(surface, area):
...
def get_array(arrayName, line, k=1):
    ...
    if k == 1:
        ...
        getData = line.GetPointData().GetArray(arrayName).GetTuple9
...
def getConnectivity(surface, mode="All", closestPoint=None):
