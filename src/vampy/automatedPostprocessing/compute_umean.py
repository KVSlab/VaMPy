...
def compute_umean(folder, nu, dt, velocity_degree, T, times_to_average, save_frequency,
    ...
    dataset_dict,  number_of_files = \
            get_files_for_cycle_averaging(dataset_us, file_counters, file_us,  number_of_cycles,
    ...
    for time_to_average, dataset in dataset_dict.items():
        ...
        define_functions_and_iterate_dataset(folder, file_counters, file_us, time_to_average, dataset,
                                              dt,
                                              mesh, nu, number_of_files, DG, V, Vv,
...
def define_functions_and_iterate_dataset(folder, file_counters, file_us, time_to_average, dataset,  dt,
                                          mesh, nu, number_of_files, DG, V, Vv, cycles_to_average,
                                         saved_time_steps_per_cycle):
    """
    ...
    metric_names = ["u_time_avg"]

    metric_variables_cycle_avg = [u_time_cycle_avg]

    metric_variables_avg = [u_time_avg ]
    ...
    for k, data in zip(file_counters, dataset):
        ...
        u_time_cycle_avg.vector().axpy(1, u.vector())
        t0.stop()
...
def main_mean():
    ...
    compute_umean(folder, nu, dt, velocity_degree, T, times_to_average, save_frequency,
                                        start_cycle, step, average_over_cycles)
