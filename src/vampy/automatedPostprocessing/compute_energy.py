...
def compute_energy(folder, nu, dt, velocity_degree, T, times_to_average, save_frequency,
    ...
    if MPI.rank(MPI.comm_world) == 0:
        print("Reading dataset names for Compute_energy.py")
...
def define_functions_and_iterate_dataset(time_to_average, dataset, dataset_avg, dt, file_u, file_path_u_avg,
    ...
    for data, data_avg in zip(dataset, dataset_avg):
        ...
        # Write instantaneous KE and TKE
        kinetic_energy.rename("Kinetic energy", "Kinetic energy")
        turbulent_kinetic_energy.rename("Turbulent kinetic energy", "Turbulent kinetic energy")

        indices['KE'].write(kinetic_energy, dt * counter)
        indices['TKE'].write(turbulent_kinetic_energy, dt * counter)
...
def main_energy():
    ...
    compute_energy(folder, nu, dt, velocity_degree, T, times_to_average, save_frequency,
                   start_cycle, step, average_over_cycles)
