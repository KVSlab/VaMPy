...
def run_simulation(config_path, local_dir, case_name):
    ...
    if not exists(sftp, case_name + "_probe_point"):
        sftp.put(
            path.join(local_dir, case_name + "_probe_point"), case_name + "_probe_point"
        )
    if not exists(sftp, case_name + "_info.json"):
        sftp.put(
            path.join(local_dir, case_name + "_info.json"), case_name + "_info.json"
        )
    ...
    stdin, stdout, stderr = client.exec_command(
        "sbatch {}".format(path.join(remote_vampy_folder, job_script))
    )
