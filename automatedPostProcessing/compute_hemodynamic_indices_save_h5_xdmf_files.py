...
def compute_hemodynamic_indices(case_path, nu, rheology_model, rho, dt, velocity_degree):
    ...
    common_path = path.join(case_path_original, "h5_files")
    if MPI.rank(MPI.comm_world) == 0:
        if not path.exists(common_path):
            makedirs(common_path)

    file_WSS = path.join(common_path, "WSS.h5")
    file_TAWSS = path.join(common_path, "TAWSS.h5")
    file_OSI = path.join(common_path, "OSI.h5")
    file_RRT = path.join(common_path, "RRT.h5")
    file_ECAP = path.join(common_path, "ECAP.h5")
    file_TWSSG = path.join(common_path, "TWSSG.h5")
    files = {"WSS": file_WSS, "TAWSS": file_TAWSS, "OSI": file_OSI, "RRT": file_RRT, "ECAP": file_ECAP, "TWSSG": file_TWSSG}
    #------------------------------------------------------------------------------------------------------------
    # Create folder to store xdmf files
    common_path_xdmf = path.join(case_path_original, "xdmf_files")
    if MPI.rank(MPI.comm_world) == 0:
        if not path.exists(common_path_xdmf):
            makedirs(common_path_xdmf)
    case_path = Path(common_path_xdmf)
    #------------------------------------------------------------------------------------------------------------
    ...
    while True:
        ...
        # Save instantaneous WSS as XDMF file
        tau.rename("WSS", "WSS")
        wss_writer.write(tau, tstep*dt)

        #--------------------------------------------------------------
        # Save instantaneous WSS as h5 file
        # Get save paths
        file_mode = "w" if tstep == t1 else "a"
        WSS_path  = files['WSS']
        viz_WSS_h5 = HDF5File(MPI.comm_world, WSS_path, file_mode = file_mode)
        viz_WSS_h5.write(tau, "/instantaneous_WSS", tstep)
        viz_WSS_h5.close()
        #--------------------------------------------------------------
    ...
    if save:
        # Save OSI and RRT
        #case_path = case_path + "/xdmf_files"
        ...
        #--------------------------------------------------------------
        # Save RRT as h5 file
        viz_RRT_h5 = HDF5File(MPI.comm_world, files["RRT"], "w")
        viz_RRT_h5.write(RRT, "/RRT")
        viz_RRT_h5.close()
        #--------------------------------------------------------------
        # Save OSI as h5 file
        viz_OSI_h5 = HDF5File(MPI.comm_world, files["OSI"], "w")
        viz_OSI_h5.write(OSI, "/OSI")
        viz_OSI_h5.close()
        #--------------------------------------------------------------
        # Save ECAP as h5 file
        viz_ECAP_h5 = HDF5File(MPI.comm_world, files["ECAP"], "w")
        viz_ECAP_h5.write(ECAP, "/ECAP")
        viz_ECAP_h5.close()
        #--------------------------------------------------------------

    # Save TAWSS and TWSSG
    ...
    #--------------------------------------------------------------
    # Save TAWSS as h5 file
    viz_TAWSS_h5 = HDF5File(MPI.comm_world, files["TAWSS"], "w")
    viz_TAWSS_h5.write(TAWSS, "/TAWSS")
    viz_TAWSS_h5.close()
    #--------------------------------------------------------------
    # Save ECAP as h5 file
    viz_TWSSG_h5 = HDF5File(MPI.comm_world, files["TWSSG"], "w")
    viz_TWSSG_h5.write(TWSSG, "/TWSSG")
    viz_TWSSG_h5.close()
    #--------------------------------------------------------------
