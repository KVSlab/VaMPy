...
def define_functions_and_iterate_dataset(time_to_average, dataset, dataset_avg, dt, file_u, file_path_u_avg,
    ...
    path_tke = fullname % "tke"
    path_teps = fullname % "teps"
    tke_writer = XDMFFile(MPI.comm_world, path_tke)
    teps_writer = XDMFFile(MPI.comm_world, path_teps)
    for writer in [tke_writer, teps_writer]:
    ...
    for data, data_avg in zip(dataset, dataset_avg):
        ...
        # Store TKE and turbulent dissipation
        kinetic_energy.rename("KE", "KE")
        tke_writer.write(kinetic_energy, dt * counter)
        dissipation.rename("Eps", "Eps")
        teps_writer.write(dissipation, dt * counter)
